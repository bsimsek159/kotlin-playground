 ...
 *
 *  For example, if Gary's path is s= [DDUUUUDD], he first enters a valley 2 units deep. Then he climbs out an up onto a 2 mountain units high. Finally, he returns to sea level and ends his hike.
 *
...
fun countingValleys(n: Int, s: String): Int {
    ...
    s.forEach {
        when (it) {
            'U' -> {
                if(direction == -1) valleyCount += 1
                direction += 1
            }
            'D' -> {
                if(direction == 1) mountainCount +=1
                direction -= 1
            }
        }
    }
